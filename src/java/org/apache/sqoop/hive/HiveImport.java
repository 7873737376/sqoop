...
public class HiveImport {
	...
	private void removeTempLogs(Path tablePath) throws IOException {
		...
		if( !tablePath.toUri().toString().startsWith("file:") && !tablePath.toUri().toString().startsWith("/")) {
			...
			conf.set( "fs.default.name", tablePath.toUri().toString() );
	...
	public String getImportTable(String inputTableName, String outputTableName,
	...
	private void cleanUp(Path outputPath) throws IOException {
		...
		if( !outputPath.toUri().toString().startsWith("file:") && !outputPath.toUri().toString().startsWith("/")) {
			Configuration conf = new HdfsConfiguration();
			conf.set( "fs.default.name", outputPath.toUri().toString() );
			conf.set( "fs.s3n.impl", "org.apache.hadoop.fs.s3native.NativeS3FileSystem" );
			conf.set( "fs.s3.impl", "org.apache.hadoop.fs.s3.S3FileSystem" );
			conf.set( "fs.file.impl", "org.apache.hadoop.fs.LocalFileSystem" );
			conf.set( "fs.hsftp.impl", "org.apache.hadoop.hdfs.HsftpFileSystem" );
