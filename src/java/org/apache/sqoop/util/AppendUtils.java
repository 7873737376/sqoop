...
public class AppendUtils {
	...
	public void append() throws IOException {
		...
		FileSystem fs2 = FileSystem.get(options.getConf());
		...
		if( !userDestDir.toUri().toString().startsWith("file:") && !userDestDir.toUri().toString().startsWith("/")) {
			...
			conf.set( "fs.default.name", userDestDir.toUri().toString() );
			...
			conf.set( "fs.hsftp.impl", "org.apache.hadoop.hdfs.HsftpFileSystem" );
			conf.set( "fs.ftp.impl", "org.apache.hadoop.fs.ftp.FTPFileSystem" );
			conf.set( "fs.har.impl", "org.apache.hadoop.fs.HarFileSystem" );
			/**
			 * Avoid socket exception when refresh sample
			 */
			...
			// move files
			moveToHDFS(fs, tempDir, userDestDir, nextPartition);
		}
		else{
			// Create target directory.
			if (!fs.exists(userDestDir)) {
	 ...
	 * NFLABS move
	 */
	private void moveToHDFS(FileSystem fs, Path sourceDir, Path targetDir, int partitionStart) throws IOException{
		...
		Configuration conf = new HdfsConfiguration();
		conf.set( "fs.default.name", targetDir.toUri().toString() );
		conf.set( "fs.s3n.impl", "org.apache.hadoop.fs.s3native.NativeS3FileSystem" );
		conf.set( "fs.s3.impl", "org.apache.hadoop.fs.s3.S3FileSystem" );
		conf.set( "fs.file.impl", "org.apache.hadoop.fs.LocalFileSystem" );
		conf.set( "fs.hsftp.impl", "org.apache.hadoop.hdfs.HsftpFileSystem" );
		...
		for (FileStatus fileStat : tempFiles) {
			if (!fileStat.isDir()) {
				...
				if (mat.matches()) {
					...
					Path path = new Path(targetDir, fileToMove);
					BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(fs2.create(path, true)));

					InputStream ins = fs.open( fileStat.getPath()  );
